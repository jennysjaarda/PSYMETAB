...
summarize_meta_results <- case_only_interest_summarize %>% dplyr::select(variable, outcome, snp, BETA_META, P_META, Q_META) %>% rename(SNP=snp) %>% dplyr::filter(outcome!="slope_weight_6mo") %>% dplyr::filter(outcome!="slope_weight") %>%
  left_join(., case_only_interest %>% dplyr::select(SNP, A1, A2, CHR, BP)) %>%
  dplyr::select(variable, outcome, SNP, CHR, BP, A1, A2, everything()) 
```
...
- We also ensured that the gw-significant SNPs we identified obtained consistent results with similar endpoint in PSYMETAB. 
...
for(i in 1:dim(case_only_interest_summarize)[1]){
  SNP_name <- case_only_interest_summarize$snp[i]
  data_sub <- subgroup_extract_snpwise[[SNP_name]] %>% mutate(variable = str_extract(string=pheno, pattern="[^_]*")) %>% 
    mutate(outcome = str_replace(pheno, paste0(variable, "_"), "")) %>% filter(eth=="CEU") %>% filter(variable=="BMI") %>%
    mutate_if(is.numeric, pretty_round)  %>% dplyr::select(eth, drug_class, drug, outcome, A1, A2, A1_CT, A1_FREQ, OBS_CT, BETA, SE, P)
 ...
 #cat(sprintf("\n\n## %s\n\n", SNP_name))
  ...
  cat(knitr::knit_print(DT::datatable(data_sub, width = "100%")))
  cat("\n\n")
#   subgroup_extract_snpwise[[SNP_name]] %>% kable() %>%
#    kable_styling(full_width = F, position = "left") %>%
#     htmltools::HTML() %>% 
#     print
#   cat("\n")
